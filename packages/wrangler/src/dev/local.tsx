...
import { fetch } from "undici"; // TODO: remove and rebase
...
export async function transformMf2OptionsToMf3Options({
	...
	if (kvRemote && authenticatedAccountId !== undefined) {
		const { Response } = await getMiniflare3();
		...
		cloudflareFetch = async (resource, searchParams, init) => {
			...
			const res = await performApiFetch(resource, init, searchParams);
			// Convert `undici`'s `Response` to `@miniflare/tre`'s
